...
import 'wave_two_variant_run_screen.dart';
...
class _VariantModeScreenState extends State<VariantModeScreen> {
  ...
  Future<void> _start({bool pressureGrid = false}) async {
    ...
    } else if (widget.mode.index >= ReactVariantMode.harpoon.index) {
      runScreen = WaveTwoVariantRunScreen(mode: widget.mode);
    } else {
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _Hero(mode: widget.mode, accent: accent, compact: compact),
                  ...
                  if (isMosaic) ...[
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.icon(
                        ...
                        label: const Text('START MODE', style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.2)),
                      ),
                    ),
                ],
              ),
...
class _Hero extends StatelessWidget {
  const _Hero({required this.mode, required this.accent, required this.compact});
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: accent.withValues(alpha: .16), blurRadius: 22)],
                  ),
                  child: Icon(mode.icon, color: accent, size: compact ? 31 : 37),
                ),
                const Spacer(),
                Container(
                  ...
                  child: FittedBox(
                    ...
                    child: Text(mode.badge, style: TextStyle(color: accent, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1.1)),
                  ),
                ),
              ],
            ),
            ...
            FittedBox(
              ...
              child: Text(mode.title, style: const TextStyle(color: ReactColors.textPrimary, fontSize: 34, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
            ),
            const SizedBox(height: 7),
            Text(mode.subtitle, style: const TextStyle(color: ReactColors.textSecondary, fontSize: 15, height: 1.35, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            Text(mode.detail, style: TextStyle(color: accent, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: .9)),
          ],
        ),
      );
}
...
class _ModeButton extends StatelessWidget {
  const _ModeButton({required this.title, required this.subtitle, required this.icon, required this.color, required this.filled, required this.onTap});
  ...
  Widget build(BuildContext context) {
    final content = Row(
      children: [
        Icon(icon, color: filled ? Colors.black : color, size: 25),
        ...
        Expanded(
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(title, style: TextStyle(color: filled ? Colors.black : color, fontSize: 13, fontWeight: FontWeight.w900, letterSpacing: .8)),
              const SizedBox(height: 2),
              Text(subtitle, style: TextStyle(color: (filled ? Colors.black : color).withValues(alpha: .72), fontSize: 7.5, fontWeight: FontWeight.w900, letterSpacing: .65)),
            ],
          ),
        ),
        Icon(Icons.chevron_right_rounded, color: filled ? Colors.black : color),
      ],
    );
    return SizedBox(
      height: 70,
      child: filled
          ? FilledButton(onPressed: onTap, style: FilledButton.styleFrom(backgroundColor: color, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(22))), child: content)
          : OutlinedButton(onPressed: onTap, style: OutlinedButton.styleFrom(foregroundColor: color, side: BorderSide(color: color.withValues(alpha: .70), width: 1.6), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(22))), child: content),
    );
  }
}

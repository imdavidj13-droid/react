alter table public.react_season_events
  ...
  add column if not exists successful_commands integer not null default 0,
  add column if not exists pb_scope text,
  add column if not exists completed_at timestamptz,
  add column if not exists personal_best_credit_awarded boolean not null default false,
  add column if not exists daily_credit_awarded boolean not null default false;
...
update public.react_season_events
set mode = case
  when event_id like 'season-classic-%' then 'classic'
  when event_id like 'season-blitz-%' then 'blitz'
  when event_id like 'season-endless-%' then 'endless'
  when event_id like 'season-daily-%' then 'daily'
  when event_id like 'season-passIt-%' then 'passIt'
  when event_id like 'season-sequence-%' then 'sequence'
  else mode
end
where mode is null;
...
begin
  if not exists (
    ...
    where conname = 'react_season_events_mode_check'
      and conrelid = 'public.react_season_events'::regclass
  ) then
    alter table public.react_season_events
      add constraint react_season_events_mode_check
      check (mode is null or mode in ('classic','blitz','endless','daily','passIt','sequence'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'react_seasons_no_overlap'
      and conrelid = 'public.react_seasons'::regclass
  ) then
    alter table public.react_seasons
      add constraint react_seasons_no_overlap
      exclude using gist (tstzrange(starts_at, ends_at, '[)') with &&);
  end if;
end $$;

create index if not exists react_season_events_pb_idx
  on public.react_season_events (player_id, season_id, pb_scope, score desc);
create index if not exists react_season_events_daily_idx
  on public.react_season_events (player_id, season_id, mode, completed_at);

drop function if exists public.record_react_season_run(
  text, integer, integer, boolean, boolean, timestamptz
);

create function public.record_react_season_run(
  p_event_id text,
  p_mode text,
  p_score integer,
...
revoke all on function public.record_react_season_run(
  text, text, integer, integer, boolean, text, timestamptz
